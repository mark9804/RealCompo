...
from grounding_input.text_grounding_tokinzer_input import GroundingNetInput
...
class UNetModel(nn.Module):
    def __init__(
        ...
        self.grounding_tokenizer_input = GroundingNetInput  # set externally
    ...
    def forward(self, input):
        ...
        for module in self.input_blocks:
            ...
            hs.append(h)
            for mdl in module:
                if isinstance(mdl, SpatialTransformer):
                    attention = mdl.get_attention_map(h, context, objs)
                    if attention.shape[1] == 256:
                        attent = attention.reshape(1, -1, 16, 16, attention.shape[-1])[0]
                        attention_map.append(attent)
            ...
            for mdl in module:
                if isinstance(mdl, SpatialTransformer):
                    ...
                    if attention.shape[1] == 256:
                        attent = attention.reshape(1, -1, 16, 16, attention.shape[-1])[0]
                        attention_map.append(attent)
        out = th.cat(attention_map, dim=0)
        out = out.sum(0) / out.shape[0]
        return self.out(h), out.cpu()
