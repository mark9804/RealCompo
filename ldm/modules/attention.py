    ...
    def fill_inf_from_mask(self, sim, mask):
        if mask is not None:
            ...
            mask = (
                ...
                .reshape(B * self.heads, 1, -1)
            )
            max_neg_value = -torch.finfo(sim.dtype).max
            sim.masked_fill_(~mask, max_neg_value)
        return sim

    def get_attention_map(self, x, key, mask=None):
...
class GatedSelfAttentionDense(nn.Module):
    ...
    def forward(self, x, objs):
        ...
        if (
            ...
            # 分块处理以节省内存
            alpha_scale = self.scale * torch.tanh(self.alpha_attn)

            # 规范化输入
            combined = torch.cat([x, objs], dim=1)
            normalized = self.norm1(combined)

            # 分块计算注意力
            chunk_size = min(128, N_visual)  # 根据可用内存调整块大小
            result = torch.zeros_like(x)

            for i in range(0, N_visual, chunk_size):
                end = min(i + chunk_size, N_visual)
                # 只计算当前块的注意力
                chunk_attn = self.attn(normalized)[:, i:end, :]
                result[:, i:end, :] = chunk_attn

            x = x + alpha_scale * result
        else:
            # 原始计算方式
            x = (
                    x
...
class BasicTransformerBlock(nn.Module):
    def __init__(
        ...
        if fuser_type == "gatedSA":
            ...
            self.fuser = GatedCrossAttentionDense(
                query_dim, key_dim, value_dim, n_heads, d_head
            )
        else:
            assert False
...
class SpatialTransformer(nn.Module):
    def __init__(
        ...
        self.proj_in = nn.Conv2d(
            in_channels, query_dim, kernel_size=1, stride=1, padding=0
        )
        ...
        self.proj_out = zero_module(
            nn.Conv2d(query_dim, in_channels, kernel_size=1, stride=1, padding=0)
        )
    ...
    def get_attention_map(self, x, context, objs):
        ...
        for block in self.transformer_blocks:
            attention_map = block.get_attention_map(x, context, objs)
