...
class PLMSSampler(object):
    ...
    def make_schedule(
        ...
        self.register_buffer(
            "alphas_cumprod_prev", to_torch(self.diffusion.alphas_cumprod_prev)
        )
    ...
    def get_latent(self, input, i, t, e_t, index):
        ...
        def get_x_prev_and_pred_x0(e_t, index):
            ...
            a_prev = torch.full(
                (b, 1, 1, 1), self.ddim_alphas_prev[index], device=self.device
            )
            sigma_t = torch.full(
                (b, 1, 1, 1), self.ddim_sigmas[index], device=self.device
            )
            sqrt_one_minus_at = torch.full(
                (b, 1, 1, 1), self.ddim_sqrt_one_minus_alphas[index], device=self.device
            )
    ...
    @torch.no_grad()
    def plms_sampling(
            self, shape, input, uc=None, guidance_scale=1, mask=None, x0=None
    ):
        ...
        scale_range = np.linspace(
            self.scale_range[0], self.scale_range[1], len(time_range)
        )
        old_eps = []
        ...
        for i, step in enumerate(tqdm(time_range)):
            ...
            with torch.enable_grad():
                influence_layout = (
                    influence_layout.clone().detach().requires_grad_(True)
                )
                influence_text = influence_text.clone().detach().requires_grad_(True)
                e_t_layout, e_t_text = self.get_epsilon(ts, input)
                e_t = self.compose_epsilon(
                    e_t_layout, e_t_text, influence_layout, influence_text
                )
                ...
                grad_cond_influence_layout = torch.autograd.grad(
                    loss.requires_grad_(True), [influence_layout], retain_graph=True
                )[0]
                grad_cond_influence_text = torch.autograd.grad(
                    loss.requires_grad_(True), [influence_text], retain_graph=True
                )[0]
                ...
                # update
                influence_layout = (
                        influence_layout - step_size * grad_cond_influence_layout
                )
                influence_text = influence_text - step_size * grad_cond_influence_text

            img, e_t = self.p_sample_plms(
                input=input,
                i=i,
                t=ts,
    ...
    def p_sample_plms(
        ...
        def get_model_output(input, influence_layout, influence_text, i):
            ...
            e_t_text = self.text_unet(
                input["x"], input["timesteps"], input["context"]
            ).sample
            del attn_layout
            if uc is not None and guidance_scale != 1:
                ...
                e_t_text_uncond = self.text_unet(
                    input["x"], input["timesteps"], uc
                ).sample
                e_t_layout_uncond, _ = self.layout_unet(unconditional_input)
                e_t_layout = e_t_layout_uncond + guidance_scale * (
                        e_t_layout - e_t_layout_uncond
                )
                e_t_text = e_t_text_uncond + guidance_scale * (
                        e_t_text - e_t_text_uncond
                )
            if i < self.t0:
                e_t = e_t_layout
            else:
