...
from utils.attentionmap import (
    AttentionStore,
...
def set_alpha_scale(model, alpha_scale):
    ...
    for module in model.modules():
        if (
                type(module) == GatedCrossAttentionDense
                or type(module) == GatedSelfAttentionDense
        ):
...
def get_clip_feature(model, processor, input, is_image=False):
    ...
    if is_image:
        ...
        inputs["pixel_values"] = inputs[
            "pixel_values"
        ].cuda()  # we use our own preprocessing without center_crop
        inputs["input_ids"] = torch.tensor([[0, 1, 2, 3]]).cuda()  # placeholder
        outputs = model(**inputs)
        feature = outputs.image_embeds
        if which_layer_image == "after_reproject":
            feature = project(
                feature, torch.load("projection_matrix").cuda().T
            ).squeeze(0)
...
def prepare_batch(meta, batch=1, max_objs=30):
    ...
    for idx, (box, text_feature, image_feature) in enumerate(
            zip(meta["boundingbox"], text_features, image_features)
    ):
    ...
    out = {
        ...
        "text_masks": text_masks.unsqueeze(0).repeat(batch, 1)
                      * complete_mask(meta.get("text_mask"), max_objs),
        "image_masks": image_masks.unsqueeze(0).repeat(batch, 1)
                       * complete_mask(meta.get("image_mask"), max_objs),
        "text_embeddings": text_embeddings.unsqueeze(0).repeat(
            batch, 1, 1
        ),  # (batch, max_objs, 768)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--t0", type=int, default=None, help="when fuse text and layout"
    )
    ...
    parser.add_argument(
        "--api_key", default=None, type=str, help="the api key of GPT-4"
    )
    parser.add_argument(
        "--no_gpt", action="store_true", help="Don't use gpt to obtain the layout"
    )
    parser.add_argument(
        "--object",
        type=str,
        default=None,
        help="the set of objects mentioned in the prompt",
    )
    parser.add_argument(
        "--boundingbox",
        type=str,
        default=None,
        help="the set of bounding boxes for each object",
    )
    parser.add_argument(
        "--token_location",
        type=str,
    ...
    if args.no_gpt:
        phrases, boundingbox, token_location = (
            ast.literal_eval(args.object),
            ast.literal_eval(args.boundingbox),
            ast.literal_eval(args.token_location),
        )
    else:
        phrases, boundingbox, token_location = get_layout(
            args.user_prompt, args.api_key
        )

    meta_list = [
        dict(
            ckpt="/root/autodl-tmp/RealCompo/models/diffusion_pytorch_model.bin",
            prompt=args.user_prompt,
            phrases=phrases,
            boundingbox=boundingbox,
            token_location=token_location,
            alpha_type=[1.0, 0.0, 0.0],
            save_folder_name="generation_realcompo_v1_sd_gligen_"
                             + args.user_prompt.replace(" ", "_"),
        ),
    ]
